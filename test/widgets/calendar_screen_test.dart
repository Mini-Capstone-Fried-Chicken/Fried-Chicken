...
void main() {
  ...
  group('CalendarScreen', () {
    ...
    testWidgets('shows login message when not logged in', (tester) async {
    ...
    testWidgets('goes to selection when restore succeeds with calendars only', (tester) async {
    ...
    testWidgets('goes to schedule when restore succeeds with events', (tester) async {
      repo.restoreResult = true;
      repo.cachedCalendars = const [
        GoogleCalendarInfo(id: 'cal1', name: 'SOEN 341', isPrimary: false),
      ];
    ...
    testWidgets('connect button shows cancelled error when connect returns false', (tester) async {
    ...
    testWidgets('connect button shows failure error when connect throws', (tester) async {
      ...
      expect(find.textContaining('Failed to connect Google Calendar:'), findsOneWidget);
    });

    testWidgets('can toggle calendar selection and continue to schedule', (tester) async {
      repo.restoreResult = true;
      repo.cachedCalendars = const [
        GoogleCalendarInfo(id: 'cal1', name: 'SOEN 341', isPrimary: false),
    ...
    testWidgets('continue shows error when fetching events fails', (tester) async {
      repo.restoreResult = true;
      repo.cachedCalendars = const [
        GoogleCalendarInfo(id: 'cal1', name: 'SOEN 341', isPrimary: false),
      ];
      ...
      expect(find.textContaining('Failed to load class schedule:'), findsOneWidget);
    });
    ...
    testWidgets('back from schedule goes to selection and saves session', (tester) async {
      repo.restoreResult = true;
      repo.cachedCalendars = const [
        GoogleCalendarInfo(id: 'cal1', name: 'SOEN 341', isPrimary: false),
      ];
