...
import '../../services/indoor_maps/indoor_map_service.dart' as indoor;
...
class _OutdoorMapPageState extends State<OutdoorMapPage> {
  ...
  Future<void> _toggleIndoorMap() async {
    ...
    // Check if this building has indoor-map support
    if (indoor.indoorAssetPath(b.code) == null) {
      if (!mounted) return;
    ...
    try {
      final data = await indoor.loadIndoorMap(b.code);
      if (data == null || !mounted) return;

      setState(() {
        _showIndoor = true;
        _indoorPolygons = data.polygons;
        _indoorGeoJson = data.geojson;
        _roomLabelMarkers = data.roomLabels;
      });
    } catch (e) {
      if (!mounted) return;
  ...
  // Geometry helpers – delegated to indoor_map_service.dart
  LatLng _polygonCenter(List<LatLng> pts) => indoor.polygonCenter(pts);
