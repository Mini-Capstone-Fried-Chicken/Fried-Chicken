...
class GooglePlacesService {
  ...
  /// Builds the request body for the autocomplete endpoint, including the
  /// location bias.  Falls back to a Concordia-area bias when [location] is null.
  ...
  /// Attempts to parse one entry from the `suggestions` array into a
  /// [PlacePrediction].  Returns null and logs a warning if the entry is
  /// missing the expected `placePrediction` field or cannot be parsed.
  ...
  /// Decodes the 200 response body and converts every suggestion entry into a
  /// [PlacePrediction], skipping any that fail to parse.
